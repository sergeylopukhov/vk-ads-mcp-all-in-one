...
describe("community research store", () => {
  it("stores a snapshot only until TTL expires", async () => {
    const directory = await mkdtemp(
      join(tmpdir(), "vk-community-research-"),
    );
    try {
      let now = Date.parse("2026-07-22T12:00:00.000Z");
      const store = new CommunityResearchStore(
