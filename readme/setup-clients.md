...
Установщик запросит данные приложения, режим `readonly/write` и отдельно предложит включить поиск публичных сообществ VK. Выберите legacy OAuth: установщик откроет браузер и попросит URL страницы `oauth.vk.ru/blank.html`. Встроенное приложение предлагается по умолчанию; своё можно использовать, если у него уже есть права `groups` и `wall`. При обновлении старой установки выбор появится снова. Затем установщик напечатает каталог установки. Codex подключится автоматически.
...
При повторном запуске установщик показывает установленную и доступную версии. «Обновить» сохраняет настройки без вопросов; «установить заново» запрашивает их с начала и заменяет `.env`.
