...
Создайте файл `.env` рядом с `package.json`: скопируйте `.env.example` в `.env` и вставьте личный токен после `VK_ADS_TOKEN=`. Не добавляйте токен в конфигурацию MCP-клиента и не публикуйте файл `.env`.
