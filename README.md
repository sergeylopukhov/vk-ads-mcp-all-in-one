...
Вес совпадения начисляется за каждое вхождение термина в названии и описании. `term_weights` увеличивает вклад отдельных профессиональных терминов. `thematic_post_share` добавляет баллы пропорционально доле последних тематических постов; `activity_low_penalty` списывается, если частота публикаций ниже `min_posts_per_week`.
