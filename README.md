...
- `vk_research_communities`: основной инструмент. Принимает поисковые фразы и фильтры, автоматически анализирует активность, возвращает прошедших и не прошедших порог кандидатов и сохраняет снимок по `run_id`. По умолчанию использует универсальные правила из поисковых фраз; `scoring_rules` и `clusters` позволяют задать собственную модель.
