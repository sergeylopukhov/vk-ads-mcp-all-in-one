...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.0.5-6b7280" alt="Версия 1.0.5">
</p>
...
В режиме `write` доступны все реализованные операции записи. Перед каждой сервер проверяет данные и создаёт preview; по умолчанию `write_execute` ждёт естественное подтверждение пользователя, без ID и фиксированной фразы. «Да», «ладно», «окей» и «делай» подходят; явные отказы, например «против», «нет», «отмена» или «стоп», блокируют запись. Владелец локального профиля может явно отключить это требование через `VK_ADS_REQUIRE_WRITE_CONFIRMATION=0`, при этом preflight и reread сохраняются. Ограничения роли, текущего доступа к объекту и опубликованного контракта VK Ads не обходятся. Перед выполнением проверьте preview и ID.
