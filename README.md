...
> [!IMPORTANT]
> Для анализа публичных сообществ нужен отдельный Core VK API token `VK_API_TOKEN`. Он не заменяет credential VK Ads и не используется для рекламных операций.
...
Установщик запросит `client_id`, скрытый `client_secret`, режим работы и отдельный выбор поиска публичных сообществ VK. При включении сообществ он скрыто запросит Core VK API token; при выборе записи расширенные возможности можно настроить отдельно. Затем установщик загрузит сервер и подключит его к Codex.
...
| Сообщества VK | Поиск, metadata, анализ публичных постов, прозрачный скоринг и CSV/JSON-экспорт кандидатов. Списки участников не запрашиваются. |

## Публичные сообщества VK

При новой установке выберите «Включить поиск и анализ публичных сообществ VK?» — установщик скрыто запросит `VK_API_TOKEN`. При обновлении старой установки он отдельно предложит включить функцию и добавить токен, не требуя менять остальные настройки. Выбор «нет» оставляет функцию выключенной.

Нужен отдельный пользовательский токен Core VK API с правами `groups`, `wall` и `ads`; он не заменяет `VK_ADS_TOKEN`. Создайте приложение VK, получите пользовательский access token через OAuth и дайте приложению нужные разрешения. Официальные инструкции: [получение токена](https://dev.vk.com/ru/api/access-token/getting-started) и [права доступа](https://dev.vk.com/ru/api/access-token/permissions). Для preview/добавления в сегмент также укажите ID рекламного аккаунта `VK_API_AD_ACCOUNT_ID`; для поиска и анализа он не требуется.

Токен не попадает в ответы, audit и диагностические логи. Метаданные кешируются в памяти на 10 минут; запросы идут через отдельный limiter с повторами при временных ошибках.

Примеры read-only вызовов:
...
Ограничения: закрытые, удалённые и недоступные сообщества отмечаются флагами; полные тексты постов не возвращаются и не сохраняются. Поиск, анализ и экспорт не создают аудитории и не меняют рекламный кабинет.

В `VK_ADS_MODE=write` доступен `vk_add_approved_communities_to_segment`. Он принимает только явный список ID и ID сегмента, сначала показывает сегмент, уже раскрытые API связи, дубликаты и число изменений, затем требует обычный одноразовый `write_execute`. Кампании, группы, объявления и бюджеты этот инструмент не создаёт и не меняет; он не запускается поиском, анализом или экспортом.
