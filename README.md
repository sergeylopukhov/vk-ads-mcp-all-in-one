...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.1.6-6b7280" alt="Версия 1.1.6">
</p>
...
При новой установке выберите «Включить поиск и анализ публичных сообществ VK?» и «legacy OAuth». Установщик откроет авторизацию в браузере и попросит URL callback, сохранив токен только в локальном `.env`. Встроенное приложение выбрано по умолчанию. Вместо него можно указать своё приложение, если для него уже доступны права `groups` и `wall`. Также поддерживается VK ID OAuth для ранее настроенных приложений.

Токен Core VK API не заменяет `VK_ADS_TOKEN` и никогда не используется для VK Ads. Он не попадает в ответы, журнал аудита и диагностические логи. Метаданные хранятся в памяти 10 минут; для Core VK API действует отдельный limiter с повторами при временных ошибках.
