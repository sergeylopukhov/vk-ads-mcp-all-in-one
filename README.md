...
Установщик запросит `client_id`, скрытый `client_secret`, режим работы и отдельно спросит, нужен ли поиск публичных сообществ VK. Если включить функцию, он скрыто запросит токен Core VK API. Дополнительные возможности записи настраиваются отдельно. Затем установщик подключит сервер к Codex.
...
При новой установке выберите «Включить поиск и анализ публичных сообществ VK?». Установщик скрыто запросит `VK_API_TOKEN`. При обновлении старой установки он отдельно предложит включить функцию и добавить токен, не меняя остальные настройки. Выбор «нет» оставляет функцию выключенной.

Нужен отдельный пользовательский токен Core VK API с правами `groups`, `wall` и `ads`; он не заменяет `VK_ADS_TOKEN`. Создайте приложение VK, получите токен через OAuth и выдайте приложению эти права. См. [получение токена](https://dev.vk.com/ru/api/access-token/getting-started) и [права доступа](https://dev.vk.com/ru/api/access-token/permissions). Для preview и добавления в сегмент укажите ID рекламного аккаунта: `VK_API_AD_ACCOUNT_ID`. Для поиска и анализа он не нужен.

Токен не попадает в ответы, журнал аудита и диагностические логи. Метаданные хранятся в памяти 10 минут. Для Core VK API действует отдельный limiter с повторами при временных ошибках.
...
В `VK_ADS_MODE=write` доступен `vk_add_approved_communities_to_segment`. Инструмент принимает явный список ID и ID сегмента, показывает сегмент, доступные API связи, дубликаты и число изменений, затем ждёт одноразового `write_execute`. Он не создаёт кампании, группы и объявления, не меняет бюджеты и не запускается после поиска, анализа или экспорта.
