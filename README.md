...
Установщик запросит `client_id`, скрытый `client_secret`, режим работы и отдельно спросит, нужен ли поиск публичных сообществ VK. Для этой функции по умолчанию добавлено рабочее приложение с нужными доступами; при необходимости можно указать своё приложение, если у него уже есть эти доступы. Дополнительные возможности записи настраиваются отдельно. Затем установщик подключит сервер к Codex.
