...
Для обычной работы используйте один инструмент: `vk_research_communities`. Он постранично читает доступную выдачу поиска VK, удаляет дубли, фильтрует, анализирует публичные записи, считает баллы, сортирует результат и сохраняет снимок. Кандидат попадает в анализ только если хотя бы одна поисковая или включающая фраза есть в его названии либо описании; тип, география, размер и исключающие термины также отсекаются до чтения стены. Без `limit` все кандидаты, прошедшие эти фильтры, анализируются последовательно пакетами по 100; укажите `limit` только если сознательно хотите ограничить объём работы. При `min_members` поиск сначала запрашивается по убыванию числа участников и прекращается после первой страницы, целиком оказавшейся ниже порога. Если конкретный Core VK API-токен возвращает на такой сортировке пустую выдачу, сервер автоматически повторяет запрос по релевантности и продолжает локальную фильтрацию. Если выдача VK или явно заданный лимит не дали охватить всё, в `summary.incomplete_reasons` это будет явно указано. В `summary` есть размер и число пакетов анализа. В ответе также есть количество найденных, отобранных и реально проанализированных сообществ, прошедшие и не прошедшие порог, настройки, версия скоринга и `run_id`. Снимок можно повторно получить через `vk_get_community_research_run` без нового обращения к VK; по умолчанию он хранится локально 30 дней, срок задаётся `VK_COMMUNITY_RESEARCH_TTL_DAYS` от 1 до 90. Экспорт не создаёт аудитории и не добавляет сообщества в сегменты.
