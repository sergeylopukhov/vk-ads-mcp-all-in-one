...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/version-1.2.6-6b7280" alt="Версия 1.2.6">
</p>
...
- `vk_research_communities`: основной инструмент. Принимает поисковые фразы и фильтры, автоматически анализирует активность, сохраняет снимок по `run_id` и отправляет уведомление о ходе раз в минуту, а затем итоговое уведомление. Без `scoring_rules` применяет универсальный профиль: нормированные совпадения в названии, описании и постах, свежесть, долю тематических публикаций, штрафы за низкую тематичность и низкую активность, исключающие термины. По умолчанию `passed` начинается с 60, а диапазон 45–59 попадает в `review`. Неполные `scoring_rules` меняют только явно заданные параметры; `clusters` добавляют пользовательскую классификацию и могут требовать минимальную тематичность, активность и отсутствие заданных рисков.
...
- `vk_rescore_community_research_run`: пересчитывает сохранённый завершённый запуск по новому профилю без нового поиска, чтения постов или запросов к VK. Создаёт новый `run_id`, сохраняет связь с исходным запуском и заново формирует вычисляемые флаги риска вместе с баллом.
