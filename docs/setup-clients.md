...
Опросник устанавливается только после вашего согласия. Его репозиторий клонируется во временную папку, после чего запускается универсальный установщик:
...
Для ручного подключения другого MCP-клиента укажите один локальный процесс по протоколу `stdio`:
